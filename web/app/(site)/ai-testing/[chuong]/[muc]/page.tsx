...
import { getSection, adjacentSections } from "@/lib/ai-testing";
import { AiTestingSidebar } from "@/components/AiTestingSidebar";
import { sectionContent } from "@/content/ai-testing/registry";
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    title: `${res.section.code} ${res.section.title} — AI Testing (CT-AI)`,
  };
}
        ...
        <div className="min-w-0">
          <article className="rounded-3xl bg-[color:var(--bg2)] text-[color:var(--ink)] px-6 py-8 md:px-10 md:py-10 ring-1 ring-white/10 shadow-2xl shadow-black/40">
            {Content ? (
              ...
              <div>
                <div className="badge">
                  {chapter.emoji} CT-AI · Chương {chapter.num} · Mục{" "}
                  {section.code}
                </div>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {prev ? (
              <Link
                href={`/ai-testing/${prev.chapter.slug}/${prev.section.slug}`}
            ...
            {next ? (
              <Link
                href={`/ai-testing/${next.chapter.slug}/${next.section.slug}`}
