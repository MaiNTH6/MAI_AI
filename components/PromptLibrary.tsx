    ...
    <div className="rounded-2xl bg-white ring-1 ring-white/10 shadow-lg shadow-black/20 overflow-hidden">
      <div className="flex items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-5 py-3">
        ...
        <button
          type="button"
          onClick={copy}
          className="shrink-0 rounded-md bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 ring-1 ring-slate-300 hover:ring-brand-400 hover:text-brand-700"
        >
          {copied ? "✓ Đã copy" : "📋 Copy"}
        </button>
      </div>
      <pre className="px-5 py-4 text-sm text-slate-800 leading-relaxed whitespace-pre-wrap font-mono">
        {prompt}
      </pre>
    </div>
  );
